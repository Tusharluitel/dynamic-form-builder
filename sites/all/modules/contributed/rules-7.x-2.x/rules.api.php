 ...
 *     rules when the rules evaluation ends. This is only supported for saveable
 ...
 *     by rules when the rules evaluation ends. Only possible for saveable data
...
function hook_rules_config_presave($config) {
  if ($config->id && $config->owner == 'your_module') {
    ...
    $config->condition(/* Your condition */);
  }
}
